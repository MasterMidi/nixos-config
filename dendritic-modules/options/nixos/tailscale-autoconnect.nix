  ...
  flake.nixosModules.tailscale-autoconnect =
    ...
    in
    {
      ...
      options.services.tailscale.autoConnect = {
        enable = lib.mkEnableOption "Enable the autoconnect to tailscale on devices";
      ...
      config = lib.mkIf (cfg.enable && config.services.tailscale.enable) {
        ...
        systemd.services.tailscale-autoconnect = {
          ...
          script = ''
            ...
            status="$(${pkgs.getExe package} status -json | ${pkgs.getExe pkgs.jq} -r .BackendState)"
            if [ $status = "Running" ]; then # if so, then do nothing
              exit 0
            fi

            # otherwise authenticate with tailscale
            ${pkgs.getExe package} up -authkey $(cat ${cfg.apiKeyFile})
          '';
        };
      };
    };
